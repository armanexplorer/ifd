...
macro "Calculate Ratio [q]" {
	calculateRatio();
}

macro "Calculate Ratio Action Tool - C0a0T0c15cT6c15aTec15l" {
	calculateRatio();
}

macro "Calculate Ratio" {
	calculateRatio();
}

function calculateRatio() {
	if (nResults < 2)
		showMessage("<html><font color=red>You should at least have 2 results to calculate the ratio!</font></html>");
	else {
		total = getResult("Area", 0);
		intended = getResult("Area", 1);
		if (isNaN(total) || isNaN(intended))
			showMessage("<html><font color=red>Error! Please try again<font></html>");
		else
			print(intended/total*100 , " %");
	}

	// Select ``Log`` window to show the recorded result to the user
	selectWindow("Log");
}